...
const CartSidebar = forwardRef<CartSidebarRef, CartSidebarProps>(
  ({ items = [], total, isOpen, onClose }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      getDestinationRect: (index: number) => {
        ...
        // If we have existing children, use the last one to find the next position
        ...
        if (lastItem) {
          ...
          // If no DOM elements but index > 0 (race condition where React hasn't flushed updates yet but we are asking for index 5)
          // We estimate based on index
