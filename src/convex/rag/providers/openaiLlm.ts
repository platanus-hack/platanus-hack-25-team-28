...
import { LLMProvider, Product, RecommendationResult } from "../types"
...
export class OpenAILLMProvider implements LLMProvider {
  ...
  private formatProductsAsContext(products: Product[]): string {
    return products
      .map((p) => {
        const priceRange = p.minPrice
          ? `$${p.minPrice.toFixed(2)}–$${p.maxPrice?.toFixed(2)}`
          : "price unavailable"
        const inStock = p.prices?.some((price) => price.inStock) ? "In stock" : "Out of stock"
        const stores = p.prices
          ?.map((price) => `${price.storeName} ($${price.currentPrice.toFixed(2)})`)
          .join(", ")
