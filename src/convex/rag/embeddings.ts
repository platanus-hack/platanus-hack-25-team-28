...
export const generateEmbeddings = action({
    ...
    handler: async (ctx, args) => {
        ...
        const result = await ctx.runQuery(internal.rag.embeddings.getProductsBatch, {
            cursor: args.cursor,
            limit,
        })
        ...
        if (productsToEmbed.length > 0) {
            console.log(`Generating embeddings for ${productsToEmbed.length} products (batch size: ${products.length})...`)
            ...
            console.log(`Batch of ${products.length} processed, no embeddings needed.`)
        }
